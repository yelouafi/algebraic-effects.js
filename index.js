...
function withHandler(handler, computation) {
...
function resume(computation, result) {
    if(isGenerator(computation)) {
...
function startOp(op) {
    ...
    const handlerComp = fh(
        op.data, 
        function resumeCont(value) {
            return cps(parentComp => {
                if(parentComp)
                    currentComp._return = parentComp
